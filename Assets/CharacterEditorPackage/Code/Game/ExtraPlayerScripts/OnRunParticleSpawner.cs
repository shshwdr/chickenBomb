...
public class OnRunParticleSpawner : MonoBehaviour {
	...
	void FixedUpdate () 
	{
        if (GetComponentInParent<CharacterController2D>().IsGrounded() && math.abs( GetComponentInParent<CharacterController2D>().rigidBody().velocity.x)>0.5f)
        {
           ...
           // float dot = Vector3.Dot(currentVel, CState.GetDirectionAlongNormal(currentVel, groundedInfo.GetNormal()));
            ...
            //if (dot >= m_LowThreshold)
            {
                ...
                //m_ParticleSystem.transform.position = transform.position;//groundedInfo.GetPoint();
                m_ParticleSystem.transform.LookAt(m_ParticleSystem.transform.position +Vector3.up/* new Vector3(groundedInfo.GetNormal().x,Vector3.up/* groundedInfo.GetNormal().y, 0.0f)*/, Vector3.back);
